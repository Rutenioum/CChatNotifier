...
local function AddTooltip(element, text)
    ...
    element:SetScript("OnEnter", function(self) 
        GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT")
        GameTooltip:SetText(text, 1, 0.9, 0.9, 1, true)
    end)
    element:SetScript("OnLeave", GameTooltip_Hide)
end
...
local function CheckBoxOnClick(self)
    ...
    svTable[self.settingName] = self:GetChecked(); -- 直接保存到全局设置
    if AfterSetCallback then AfterSetCallback(); end -- 触发回调
end
...
function settings:MakeCheckboxOption(settingName, labelText, tooltipText, row)
    ...
    cb:SetChecked(CChatNotifier_settings[settingName] or false)
...
local function EditBoxRefresh(self)	
    self:SetText(svTable[self.settingName] or ""); -- 直接从全局设置读取
    self:SetCursorPosition(0);
end

local function EditEnterPressed(self)
    self:ClearFocus();
    svTable["outputFormat"] = self:GetText(); -- 直接保存到全局设置
    if AfterSetCallback then AfterSetCallback(); end
end
...
function settings:MakeEditBoxOption(settingName, labelText, max, numeric, tooltipText, row, forceWidth, labelWidth)
    ...
    edit:SetScript("OnEnterPressed", EditEnterPressed);
    ...
    edit:SetText(svTable["outputFormat"] or ""); -- 直接从全局设置初始化
    edit:SetCursorPosition(0);  -- 光标复位
...
function settings:MakeSliderOption(settingName, labelText, tooltipText, min, max, step, row)
    ...
    -- 初始化时直接读取全局设置
    slider:SetValue(svTable[settingName] or min)
    slider.curVal:SetText(svTable[settingName] or min)
    
    -- 修改变更事件
    slider:SetScript("OnValueChanged", function(self, value)
        svTable[self.settingName] = value
...
local function DropdownOpen(self, level, menuList)
    ...
    info.func = function(selfb, arg1, arg2)
        svTable[self.settingName] = arg1; -- 直接保存到全局设置
    ...
    for k, v in pairs(self.GetListItems()) do
        ...
        info.arg1 = k;
        info.arg2 = v;
        info.checked = (k == svTable[self.settingName]); -- 直接对比全局设置
...
function settings:MakeDropdown(settingName, labelText, tooltipText, width, func, labelWidth, row)
    ...
    local initialValue = svTable[settingName]
	C_Timer.After(3, function() 
		local pinDaoString = func()[initialValue]
		--print(initialValue,pinDaoString)
		if initialValue then
			UIDropDownMenu_SetText(dropDown, pinDaoString or "")
		end 
	end)
