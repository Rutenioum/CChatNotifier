...
if InterfaceOptionsFrame_OpenToCategory == nil then
	function InterfaceOptionsFrame_OpenToCategory(_addonName)
		if type(_addonName) == "table" then
			local categoryID = _addonName.name;
			return Settings.OpenToCategory(categoryID);
		else
			return Settings.OpenToCategory(_addonName);
		end
	end
end
--- Open settings menu
