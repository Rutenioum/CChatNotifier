...
local function ChatFilter(self, event, msg, author, ...)
    -- Filter logic:
    -- If we are here, it means Acamar (if installed and loaded) has already processed the message
    -- and decided NOT to block it (returned false).
    
    local guid = select(10, ...)
    
    if event == "CHAT_MSG_CHANNEL" then
        local channelName = select(2, ...) -- arg4 in event, 2nd in ... (lang, channelName, ...)
        SearchMessage(msg, author, channelName, guid);
    else
        -- SAY, YELL
        SearchMessage(msg, author, L["VICINITY"], guid);
    end
    
    -- Always return false to let the message show in chat (unless we want to block it, which we don't)
    return false;
end
...
local function UpdateAddonState()
    if CChatNotifier_settings.isActive then
        ...
        ChatFrame_RemoveMessageEventFilter("CHAT_MSG_YELL", ChatFilter);
